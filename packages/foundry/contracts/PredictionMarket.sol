...
contract PredictionMarket {
    ...
    constructor(
        ...
        prediction.tokenRatio =
            (msg.value * PRECISION * PRECISION) /
            _initialTokenAmount;
    ...
    ) external payable onlyPredictionOpen withValidOption(_optionId) {
    ...
    function sellTokensForEth(
    ...
    ) external onlyPredictionOpen withValidOption(_optionId) {
        uint256 ethToReceive = sellTotalPriceInEth(
            _optionId,
            _tokenAmountToSell
        );
        ...
        if (_optionId == 0) {
            ...
            prediction.optionToken1.transferFrom(
                msg.sender,
                address(this),
                _tokenAmountToSell
            );
            ...
            prediction.optionToken2.transferFrom(
                msg.sender,
                address(this),
                _tokenAmountToSell
            );
    ...
    ) external onlyPredictionOpen withValidOption(_winningOption) {
    ...
    function totalPriceInEth(
        ...
        uint256 num1 = (PRECISION -
            ((currentTokenReserve * PRECISION) /
                (prediction.initialTokenAmount * 2)));
        uint256 num2 = (PRECISION -
            (((currentTokenReserve - _tradingAmount) * PRECISION) /
                (prediction.initialTokenAmount * 2)));

        // Calculate average first to reduce number size
        uint256 avgNumerator = (num1 + num2) / 2;

        uint256 avg = (prediction.tokenRatio * avgNumerator) /
            PRECISION /
            PRECISION;
